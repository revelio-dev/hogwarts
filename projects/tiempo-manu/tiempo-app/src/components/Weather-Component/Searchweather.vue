<template>
    <b-container class="bv-example-row bv-example-row-flex-cols">
        ...
        <div class="container-fluid" id="form">
            ...
            <h3><em>{{weatherDescription}}</em></h3>
            </div>
        ...
        <b-button :disabled="!query.length" @click="showAll()" variant="primary">Search</b-button>
...
<script>
    ...
    export default {
        ...
        methods: {
            showWeather(){
                this.show=true;
                this.$http.get(URL+this.query+'&?units=metric&APPID='+API_KEY).then(response => {
                    this.city = response.data.name;
                    this.country = response.data.sys.country;
                    this.weatherDescription = response.data.weather[0].description.toUpperCase();
                    this.temp = response.data.main.temp;
                    this.tempMin = response.data.main.temp_min;
                    this.tempMax = response.data.main.temp_max;
                    this.humidity = response.data.main.humidity;
                    this.cod=response.data.weather[0].icon;
                    this.icon = 'http://openweathermap.org/img/wn/'+this.cod+'@2x.png';
                    this.error = false;
